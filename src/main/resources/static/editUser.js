function editUser(id) {
    ...
    fetch("http://localhost:8080/edit", {
        ...
        body : JSON.stringify({
            ...
            name : window.updateUserFirstName.value,
            lastName : window.updateUserLastName.value,
        ...
        if (status == 'OK') {
            ...
            $('#row' + id).replaceWith(`<tr id= ${"row" + id} >
                ...
                <td id="name">${window.updateUserFirstName.value}</td>
                <td id="lastName">${window.updateUserLastName.value}</td>
